<template>
  <div
    ...
    <component
      ref="stimulus"
      @stimulusReady="handleStimulusReady"
...
export default {
  ...
  data () {
    return {
      stimulus: null,  // Set to the qti-assessment-stimulus component
      stimulusXml: '', // QTI XML string injected into the qti-assessment-stimulus component
      xmlFilters: null,
      guid: null,
  ...
  computed: {
    processedXml () {
      ...
      return {
        template: '<div id="qti-stimulus-container">' + xml + '</div>'
      }
    }
  },
  ...
  methods: {
    ...
    /**
     * @description Main stimulus loading method for the Qti3StimulusPlayer.  Accepts
     * a raw QTI 3 xml string and a configuration object.
     * @param {String} xml - string of qti-assessment-stimulus (QTI 3) xml
     ...
     * }
     */
    loadStimulusFromXml (xml, configuration) {
      console.log('[Qti3StimulusPlayer][LoadStimulusFromXml][configuration]', configuration)
      // Step 1: reset (close, re-center, etc) Catalog Dialog
      this.resetCatalogComponents()
      // Step 2: set stimulus player context
      this.loadStimulusContextFromConfiguration(configuration)
      // Step 3: load the stimulus xml
      this.stimulusXml = xml
    },
     ...
     * @param {Object} param - contains a 'stimulus' property (the qti-assessment-stimulus component)
     */
    handleStimulusReady (param) {
      // Keep a handle on the component.
      this.stimulus = param.stimulus
      // Notify listener that the qti-assessment-stimulus component is loaded and ready.
      this.$emit('notifyQti3StimulusReady', param)
    },
    ...
    handleCatalogEvent (catalogEvent) {
      ...
      this.$emit('notifyQti3StimulusCatalogEvent', catalogEvent)
    },

    /**
     * @description Method for controller to get a handle on the stimulus component.
     * @return {Component} a qti-assessment-stimulus component
     */
    getStimulus () {
      return this.stimulus
    },

    loadStimulusContextFromConfiguration (configuration) {
      if (typeof configuration === 'undefined') return

      // If the configuration properties exist, Load StimulusContext from the configuration.
      if ('guid' in configuration) this.setStimulusContextGuid(configuration.guid)
      if ('pnp' in configuration) this.setStimulusContextPnp(configuration.pnp)
    },

    /**
     * @description Method to set a tracking guid.
     * @param {String} guid - a tracking guid
     */
    setStimulusContextGuid (guid) {
      this.guid = guid
    },
     ...
     */
    bindCatalog () {
      console.log('[Qti3StimulusPlayer][BindCatalog]')
      this.resetCatalogComponents()
      this.stimulus.bindCatalog()
    },
    ...
    /**
     * @description After a stimulus is updated, we may need to do some cleanup of
     * the DOM or perform other post processing.
     */
    postProcessing () {
      this.$nextTick(() => {
        // Item XML should be rendered, run MathJax over just the Stimulus Container
        if (window.MathJax) {
  ...
  mounted () {
    console.log('[Qti3StimulusPlayer][Ready]')
    this.$emit('notifyQti3StimulusPlayerReady', this)
  }
}
</script>
